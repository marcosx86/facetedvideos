...
<div id="editcell">
	...
	for ($i=0, $n=count( $this->records ); $i < $n; $i++)
	{
		...
		$link = JRoute::_( 'index.php?option=com_facetedvideos&controller=facettypes&task=edit&cid[]='. $row->id );
		$pubact = $row->published ? "unpublish" : "publish";		
		$publink = JRoute::_( 'index.php?option=com_facetedvideos&controller=facettypes&task=' . $pubact . '&cid[]=' . $row->id );
		...
		<tr class="<?php echo "row$k"; ?>">
			...
			<td>
				<?php echo ($row->showAsColumn ? "Yes" : "No"); ?>
			</td>
			<td>
